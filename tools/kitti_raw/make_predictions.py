...
def make_predictions(sequence, root_dirname, ckpt_dirname, ckpt_filename, split_filename, class_names):
    ...
    group_filename = os.path.join(root_dirname, "filenames", split_filename, sequence.rsplit("_", 3)[0], sequence, "grouped_image_filenames.txt")

    if not os.path.exists(group_filename):
        print(f"[{group_filename}] Does not exist!")
        return
    ...
    sample_filename = os.path.join(root_dirname, "filenames", split_filename, sequence.rsplit("_", 3)[0], sequence, "sampled_image_filenames.txt")

    if not os.path.exists(sample_filename):
        print(f"[{sample_filename}] Does not exist!")
        return
    ...
    for instance_ids, grouped_image_filenames in grouped_image_filenames.items():
        ...
        target_ckpt_filename = os.path.join(ckpt_dirname, sequence.rsplit("_", 3)[0], target_image_dirname, ckpt_filename)
        ...
        target_annotation_filename = target_image_filename.replace("image", "annotations").replace(".png", ".json")
        ...
        for source_image_filename in grouped_image_filenames:
            ...
            source_annotation_filename = source_image_filename.replace("image", "annotations").replace(".png", ".json")
            ...
            source_prediction_dirname = os.path.join("predictions_02", os.path.basename(ckpt_dirname))
            source_prediction_filename = source_annotation_filename.replace("annotations_02", source_prediction_dirname)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="VSRD: Prediction Maker for KITTI-Raw")
