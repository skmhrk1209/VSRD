...
def eval_from_scrach(gt_dirs, det_dirs, ap_mode=40):
    global AP_mode
    AP_mode = ap_mode

    import glob
    gt_files = sorted(sum([glob.glob(os.path.join(gt_dir, "**", "*.txt"), recursive=True) for gt_dir in gt_dirs], []))
    det_files = sorted(sum([glob.glob(os.path.join(det_dir, "**", "*.txt"), recursive=True) for det_dir in det_dirs], []))

    all_gt, all_det = [], []
    # all_f = sorted(os.listdir(det_dir))
    for i, (gt_file, det_file) in enumerate(tqdm(zip(gt_files, det_files))):

        assert os.path.basename(gt_file) == os.path.basename(det_file)

        gt_f = np.loadtxt(gt_file, dtype=str).reshape(-1, 16)
        det_f = np.loadtxt(det_file, dtype=str).reshape(-1, 16)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="VSRD: Box Evaluator for KITTI-360")
    parser.add_argument("--gt_dirs", type=str, nargs="+")
    parser.add_argument("--det_dirs", type=str, nargs="+")
    parser.add_argument("--ap_mode", type=int, default=40)
    args = parser.parse_args()

    eval_from_scrach(args.gt_dirs, args.det_dirs, ap_mode=args.ap_mode)
